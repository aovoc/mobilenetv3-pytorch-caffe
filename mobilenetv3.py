...
class Avgpool(nn.Module):
    ...
    def forward(self, x):
        b, c, d, e = x.size()
        avg_pool = nn.AvgPool2d((d, e), stride=(1, 1))
        y = avg_pool(x)
        return y
...
class Hswish(nn.Module):
    ...
    def hsigmoid(self, x):
        return (6- F.relu(6- F.relu(x + 3) ) ) / 6.
...
class MobileNetV3(nn.Module):
    def __init__(self, n_class=1000, input_size=224, dropout=0.8, mode='small', width_mult=1.0):
        ...
        if mode == 'large':
            ...
            self.features.append(nn.Conv2d(last_conv, last_channel, 1, 1, 0, bias=False))
        ...
        # building classifier
        self.fc = nn.Linear(last_channel, n_class)
...
def mobilenetv3_small(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load('mobilenetv3_small.pth'))
    return model
